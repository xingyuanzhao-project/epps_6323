...
summary(lm.fit)
# Compute per-predictor VIF directly from model matrix.
compute_vif <- function(model) {
  ...
  if (ncol(mm) < 2) {
    return(setNames(rep(1, ncol(mm)), colnames(mm)))
  }
  vifs <- sapply(seq_len(ncol(mm)), function(i) {
    r2 <- summary(lm(mm[, i] ~ mm[, -i]))$r.squared
    1 / (1 - r2)
  })
  setNames(vifs, colnames(mm))
}
compute_vif(lm.fit)
