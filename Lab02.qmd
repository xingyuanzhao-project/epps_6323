...
ptbu=c("MASS","ISLR","car")
missing_ptbu <- ptbu[!sapply(ptbu, requireNamespace, quietly = TRUE)]
if (length(missing_ptbu) > 0) {
  install.packages(missing_ptbu, repos='https://cloud.r-project.org', lib = project_lib)
}
invisible(lapply(ptbu, library, character.only = TRUE))
